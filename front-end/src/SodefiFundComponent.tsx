...
const SodefiFundComponent = () => {
    ...
    const fetchFactsheetData = async () => {
      ...
      try {
        ...
        const formattedContributions = contributions?.map((c: { name: string; value: number | null; portfolio_type: string}) => ({
          name: c.name,
          value: c.value,
          fill: c.value && c.value > 0 ? '#928254' : '#004156', // brand-positive and brand-negative
  ...
  const getCommentaryClass = () => {
    const numParagraphs = commentaryParagraphs.length;
    if (numParagraphs <= 4) {
  ...
  const middleIndex = Math.ceil(commentaryParagraphs.length / 2);
  const firstColumnParagraphs = commentaryParagraphs.slice(0, middleIndex);
  const secondColumnParagraphs = commentaryParagraphs.slice(middleIndex);
    ...
    <div className="bg-white">
      <header className="flex justify-between items-center px-14 py-5">
        ...
        <div className="text-right">
          ...
          <div className="text-[10pt] text-gray-600 font-light">{factsheetDate}</div>
        </div>
      </header>
    </div>
  );
  ...
  const PageFooter = () => (
    <div className="mt-auto h-28 flex flex-col">
  ...
  const generatePdf = async () => {
    ...
    // Padding: 0.5cm top, 1.5cm horizontal, 0cm bottom
    const paddingTop = 0;
    ...
    const processPage = async (elementId: string, isFirstPage = false) => {
      ...
      pdf.setFillColor('#f5f4eb');
      pdf.rect(0, 0, pdfWidth, pdfHeight, 'F'); // 'F' fills the rectangle
      ...
      const canvas = await html2canvas(element, { 
        ...
        onclone: (document) => {
          ...
          if (clonedElement) {
            ...
            clonedElement.style.backgroundColor = 'transparent'; // Ensure transparency
          }
        }
      });
      const imgData = canvas.toDataURL('image/png');
      
      const contentWidth = pdfWidth; // Full width
      const contentHeight = pdfHeight - paddingTop - paddingBottom;

      pdf.addImage(imgData, 'PNG', 0, paddingTop, contentWidth, contentHeight, undefined, 'FAST');
    };
    ...
    <>
      <style>{`
        ...
        #page1, #page2, #page3 {
          width: 794px;
          height: 1123px;
          background-color: #f5f4eb; /* brand-background */
      ...
      `}</style>
      <div id="factsheet-container" className="font-sans px-8 py-4">
        <div id="pdf-button-container" className="text-right mb-2">
          <div className="flex justify-center items-right gap-4 mb-2">
            <select
              value={selectedYear || ''}
            ...
            className="bg-red-600 text-white font-bold py-1 px-2 rounded hover:bg-red-700 disabled:bg-gray-400"
          >
        ...
        {isLoading ? (
          ...
          <div id="pdf-container" className="border border-gray-300 shadow-lg">
            <div id="page1" className="flex flex-col text-brand-main-text">
              <PageHeader />
              <div className="px-14 py-2 flex-grow">
                ...
                <div className=" text-[7pt] text-left">
                  <p className="my-0">* Past performance is not a guarantee of future return.</p>
                  <p className="my-0">** Performance of Lead Series from 1st of November 2022, historic track record of founding investor from April 2011 to October 2022.</p>
                </div>
                ...
                <div className="my-1">
                  <div className="flex gap-4">
                      ...
                      <div className="bg-brand-graph-bg">
                        ...
                        >
                          <div className="flex items-center justify-center gap-6">
                            <div className="flex items-center gap-2">
                              <div style={{ width: '20px', height: '2px', backgroundColor: '#004156' }} />
                              <span className="font-medium">SODEFI</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <div style={{ width: '20px', height: '2px', backgroundColor: '#928254' }} />
                        ...
                        <div className="px-2 py-1 relative">
                          <ResponsiveContainer width="100%" height={200}>
                            <LineChart data={performanceData} margin={{ top: 5, right: 20, left: -30, bottom: 5 }}>
                              <XAxis 
                                dataKey="date" 
                                tickLine={false}
                                tick={(props) => {
                    ...
                    <div className="w-1/2 px-2 py-1">
                      <h2 className="text-[14pt] font-bold mb-2 text-left text-brand-subheading">General Information</h2>
                      <div className="mb-1">
                        <div className="flex mb-0 text-[10px] leading-tight">
                      ...
                      <h2 className="text-[14pt] font-bold mb-2 text-left text-brand-subheading">Administration</h2>
                      <div className="mb-1">
                        <div className="flex mb-0 text-[10px] leading-tight">
                  ...
                  <div className="mt-0 text-[7pt] leading-relaxed text-left">
                    <p className="my-0">The performance above represents a pro forma EUR result for a single investor in the fund, prior to the restatement of its Terms & Conditions to permit outside investor participation. The first fund NAV calculation was for the month of NOV 2022. The results have been independently verified.</p>
                    <p className="my-0">* The "stichting" is a foundation which is a self-contained legal entity with separate legal personality that has no members or shareholders.</p>
                    <p className="my-0">All powers within the stichting are vested in its board of Directors.</p>
                  </div>
                </div>
                

                  <h2 className="text-left mb-0 text-[14pt] text-brand-subheading font-bold">Relative Return Contribution - {factsheetDate}</h2>
                  ...
                  <p className="text-center text-[10px] text-gray-600  italic">*C.N.C = Consumer Non Cyclical *C.C = Consumer Cyclical</p>
            ...
            <div id="page2" className="flex flex-col text-brand-main-text">
              <PageHeader />
              <div className="px-14 py-4 flex-grow">
                ...
                <div className="mt-4">
                  ...
                  <div className={`grid grid-cols-2 gap-8 text-left ${getCommentaryClass()}`}>
                    <div className="space-y-4">
                      {firstColumnParagraphs.map((p, i) => <p key={i} className="mb-3">{p}</p>)}
                    </div>
                    
                    <div className="space-y-4">
                      {secondColumnParagraphs.map((p, i) => <p key={i} className="mb-3">{p}</p>)}
                    </div>
                  </div>
            ...
            <div id="page3" className="flex flex-col text-brand-main-text">
              ...
              <div className="px-14 py-2 flex-grow">
                ...
                <div className="mt-4 p-0">
                  ...
                  <div className="grid grid-cols-2 gap-8 text-[9pt] leading-relaxed text-left">
                    <div className="space-y-3">
                      ...
                      <hr className="border-t-1 border-brand-main-text my-0 pt-1" />
                      <p>
